...
exports.default = {
    ...
    execute: async (ctx) => {
        ...
        embed.addFields({
            name: "Information",
            value: ctx.join(`Uptime: ${(0, discord_js_1.bold)(ctx.util.parseMS(ctx.client.uptime))}`, `Developer: ${(0, discord_js_1.bold)(fetchedDevs.map((x) => (0, discord_js_1.hyperlink)(`@${x.username}`, `https://discord.com/users/${x.id}`)).join(" | "))}`),
        });
        await ctx.reply({
            content: "*Calculated*",
            embeds: [embed],
